...
import { Search, Bell, Settings, LogOut, Menu, UserCircle, ShieldCheck, User, ChevronRight, CheckCircle2 } from "lucide-react";
    ...
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200/60">
      <div className="flex h-16 sm:h-20 items-center justify-between px-4 sm:px-6 md:px-8">
        <div className="flex items-center gap-3 min-w-0 flex-shrink-0">
          ...
          <Link href={is_admin ? "/admin/dashboard" : "/dashboard"} className="flex items-center gap-2">
            <Image
              src="/images/logo.png"
              alt="Jomnus"
              width={240}
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <div className="relative">
                <div className={`h-9 w-9 sm:h-10 sm:w-10 rounded-full overflow-hidden bg-slate-100 relative border-2 shadow-sm ${
              ...
              {open && (
                ...
                >
                  ...
                  <div className={`px-5 py-5 flex items-center gap-4 ${
                    ...
                    <div className="relative">
                      <div className={`h-14 w-14 rounded-2xl overflow-hidden bg-slate-100 relative border-2 ${
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-widest ${
                          is_admin ? "bg-white/20 text-white backdrop-blur-md" : "bg-blue-600 text-white"
                        }`}>
                          {role}
                        </span>
                        {is_admin && (
                  ...
                  {is_admin && (
                    <div className="px-5 py-3 bg-indigo-50/50 border-b border-indigo-100 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] font-bold text-indigo-700">Pending Actions</span>
                      </div>
                      <span className="text-[11px] font-black bg-indigo-600 text-white px-2 py-0.5 rounded-full">3</span>
                    </div>
                  )}
                    ...
                    {is_admin && (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-bold text-slate-700">Admin Console</span>
                        </div>
                        <ChevronRight size={16} className="text-slate-300 group-hover:text-slate-500 transition-colors" />
                      </Link>
                    )}
